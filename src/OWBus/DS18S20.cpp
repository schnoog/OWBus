...
bool DS18S20::writeScratchpad(){
	OneWire *ow = this->getBus().getOWTechLayer();
	if(!ow->reset())
		return false;

	ow->write( this->getOWCommand( OWDevice::OWCommands::WRITE_SCRATCHPAD ) );
	ow->write( this->operator[](DS18S20::SCRATCHPAD_INDEX::HIGH_ALARM_TEMPERATURE) );
	ow->write( this->operator[](DS18S20::SCRATCHPAD_INDEX::LOW_ALARM_TEMPERATURE) );
	ow->write( this->operator[](DS18S20::SCRATCHPAD_INDEX::CONFIGURATION) );

	this->operator[](DS18S20::SCRATCHPAD_INDEX::SCRATCHPAD_CRC) = OneWire::crc8(this->getScratchpadMemory(), 8);	// Force a valid scratchpad
	return(ow->reset());
}

float DS18S20::readLastTemperature(){
	if(!this->readScratchpad() || !this->isValidScratchpad())
		return this->BAD_TEMPERATURE;

	int16_t val = (this->operator[](DS18S20::SCRATCHPAD_INDEX::TEMPERATURE_MSB) << 8) | this->operator[](DS18S20::SCRATCHPAD_INDEX::TEMPERATURE_LSB);

	switch(this->operator[](DS18S20::SCRATCHPAD_INDEX::CONFIGURATION) & 0x60){	// Clean unused bits
...
unsigned long DS18S20::getConversionDelay(){
	if(virgin && !this->readScratchpad())
		return 1000;	// By default, we will wait for a second

	switch(this->operator[](DS18S20::SCRATCHPAD_INDEX::CONFIGURATION) & 0x60){
...
bool DS18S20::setResolution(uint8_t v){
	...
	c = this->operator[](DS18S20::SCRATCHPAD_INDEX::CONFIGURATION);	// Byte storing the resolution
	c &= ~0x60;
...
bool DS18S20::launchTemperatureAcquisition(bool parasite){
...
float DS18S20::getTemperature(bool parasite){
